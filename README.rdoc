= ShortURL 1.0.0

== Summary
ShortURL is a very simple library to use URL shortening services.

== Installation
ShortURL is available as a gem, installation is just like any other gem:
<tt>$ gem install shorturl</tt>

You can also download the tar.bz2 archive at
http://rubyforge.org/frs/?group_id=732 if you do not want to use the gem.

== Supported services
Here is the list of the services supported by ShortURL:
* http://tinyurl.com
* https://cutyurl.com
* http://shorl.com
* http://snipurl.com
* http://metamark.net
* http://makeashorterlink.com
* http://skinnylink.com
* http://linktrim.com
* http://shorterlink.com
* http://minilink.org
* http://ln-s.net
* http://fyad.org
* http://d62.net
* http://littlink.com
* http://clipurl.com
* http://0rz.net
* http://moourl.com
* http://urltea.com
* http://vurl.me
* http://bit.ly
* http://is.gd
* http://git.io
* http://va.mu

== Usage:
call-seq:
  require "shorturl"
  puts ShortURL.shorten("http://mypage.com")
  puts ShortURL.shorten("http://mypage.com", :moourl)

The second parameter represents the service you want to use.  These are:
* <tt>:tinyurl</tt>
* <tt>:cutyurl</tt>
* <tt>:shorl</tt>
* <tt>:snipurl</tt>
* <tt>:metamark</tt>
* <tt>:makeashorterlink</tt>
* <tt>:skinnylink</tt>
* <tt>:linktrim</tt>
* <tt>:shorterlink</tt>
* <tt>:minlink</tt>
* <tt>:lns</tt>
* <tt>:fyad</tt>
* <tt>:d62</tt>
* <tt>:littlink</tt>
* <tt>:clipurl</tt>
* <tt>:orz</tt>
* <tt>:moourl</tt>
* <tt>:urltea</tt>
* <tt>:vurl</tt>
* <tt>:bitly</tt>**


You can use <tt>ShortURL.valid_services</tt> to obtain a
list of the valid services (in case I forget to update the
documentation)

CutyURL**
-------
🔑 1. Get Your API Key

Sign up at CutyURL and you’ll receive an API key automatically 
publicapi.dev

This key must be sent with every request in the Authorization: Bearer YOUR_API_KEY header.

2. Shorten a URL 
curl -X POST \
  'https://cutyurl.com/api/links' \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"destination":"https://example.com"}'


Bitly**
-------

   Their API changed to require an API key.

   Two painfree steps to fix this.

   1) Get yours easily right now from:

   http://bitly.com/a/your_api_key




   2) Save it in ~/.shorturl similar to this YAML template:

--- Start of file ~/.shorturl ---

   ---
   bitly:
     username: O_adsfasdfasfasfd
     key: R_afasdfasdfasdf

--- END of file ~/.shorturl ---


   You're done!  Have a martini.  Cheers.

== Thanks
- Marcel Molina Jr., Devin Mullins for some ideas
- imperator from #ruby-lang (I don't know your real name, sorry) for
  helping me with creating and uploading a RubyGem
- Daniel Dipaolo for telling me about ln-s.net and fyad.org

== Authors
- Vincent Foley (original developer)
- Robby Russell <robbyrussell@gmail.com>
